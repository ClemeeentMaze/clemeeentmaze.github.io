...
import { AIModerationGroup } from './AIModerationGroup';
...
export function BlockList({ blocks, selectedBlockId, onSelectBlock, studyMeta }) {
  ...
  /**
   * Groups consecutive blocks that share the same groupId.
   * @param {Array} blockList - List of blocks (excluding welcome/thankyou)
   * @returns {Array} Render descriptors for grouped and ungrouped blocks
   */
  const buildGroupedBlocks = (blockList) => {
    const results = [];
    ...
    blockList.forEach((block) => {
      if (block.groupId) {
        if (activeGroupId === block.groupId) {
      ...
      flushGroup();
      results.push({ type: 'block', id: block.id, block });
    });
  ...
  const groupedBlocks = buildGroupedBlocks(customBlocks);
      ...
      <ScrollContainer className="flex-1 min-h-0">
        <Flex flexDirection="column" gap="SM" className="p-3">
          ...
          {groupedBlocks.map((item) => {
            if (item.type === 'group') {
              return (
                <AIModerationGroup key={`group-${item.id}`} label="AI moderated">
