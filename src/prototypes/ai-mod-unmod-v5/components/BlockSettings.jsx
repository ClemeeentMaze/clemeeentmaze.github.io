...
import { MoreHorizontal, Info, Sparkles } from 'lucide-react';
          ...
          <>
            <Flex justifyContent="space-between" alignItems="center" className="mb-3">
              <Flex alignItems="center" gap="SM">
                <Box className="w-8 h-8 rounded-lg bg-[#F3F0FF] flex items-center justify-center">
                  <Sparkles size={16} className="text-[#6B5BEE]" />
                </Box>
                <Flex flexDirection="column" gap="XS">
                  <Text className="font-semibold">AI Moderation</Text>
                  <Text type="caption" color="default.main.secondary">
                    Convert this block to a conversational interview
                  </Text>
                </Flex>
              </Flex>
              <Toggle
                size="xs"
                checked={Boolean(block.aiModerated)}
                onChange={(value) => {
                  console.log('AI moderation toggled:', value);
                  onBlockChange?.(block.id, { aiModerated: value });
                }}
              />
            </Flex>
            <Divider />
