 ...
 */
function PreviewContent({ block, isStudyModerated }) {
  if (!block) {
    return (
 ...
 * @param {boolean} props.isStudyModerated - Study-level AI moderation state
 */
export function BlockPreview({ block, isStudyModerated }) {
  ...
  return (
    <Flex flexDirection="column" className="h-full bg-[#F8F8FB]">
      ...
      {/* Tab content */}
      {activeTab === 'preview' ? (
        <PreviewContent block={block} isStudyModerated={isStudyModerated} />
      ) : (
        <CommentsContent />
      )}
    </Flex>
