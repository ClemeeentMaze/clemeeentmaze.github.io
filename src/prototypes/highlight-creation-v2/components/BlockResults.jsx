...
import { MoreHorizontal, Filter, Play, ChevronLeft, ChevronRight, ChevronDown, ChevronUp, Table2, Highlighter, Tag, Info, Plus, Sparkles } from 'lucide-react';
...
/**
 * Expanded Response Card (V2 approach)
 * Full-width card with video player, transcript, and highlight creation
 */
function ExpandedResponseCard({ response, blockType, onClose, onCreateHighlight }) {
  const [selectedText, setSelectedText] = useState('');
  const [note, setNote] = useState('');
  const [isGenerating, setIsGenerating] = useState(false);
  const [showHighlightForm, setShowHighlightForm] = useState(false);
  
  // Mock full transcript for the response
  const fullTranscript = blockType === 'input' || blockType === 'ai_conversation' 
    ? `"${response.responseValue}" I found the overall experience quite intuitive. The navigation was clear, though I had some trouble finding the settings menu at first. Once I figured that out, everything else made sense. I particularly liked how the feedback was immediate when I made changes.`
  ...
  const handleGenerateNote = () => {
    ...
    setTimeout(() => {
      setNote(`The participant highlights "${selectedText.slice(0, 40)}..." as a key insight about their experience with the product.`);
      setIsGenerating(false);
    }, 1000);
  };

  return (
    <div className="bg-white border border-[rgba(108,113,140,0.28)] rounded-xl p-6 mb-4 shadow-sm">
      <Flex alignItems="center" justifyContent="space-between" className="mb-4">
        ...
        <ActionButton emphasis="tertiary" size="SM" onClick={onClose} icon={<ChevronUp size={16} />}>
          Collapse
        </ActionButton>
      </Flex>
      
      <Flex gap="LG" className="mb-4">
        {/* Video Player Area */}
        <div className="w-[400px] flex-shrink-0">
          ...
          <Text color="default.main.secondary" className="text-xs mt-2">
            Click and drag on the timeline to select a portion for highlight
          </Text>
        </div>
        ...
        <div className="flex-1">
          <Flex alignItems="center" justifyContent="space-between" className="mb-3">
            ...
            <Text color="default.main.secondary" className="text-xs">Select text to create a highlight</Text>
          </Flex>
          <div 
      ...
      {/* Highlight Creation Form - shows when text is selected */}
      {showHighlightForm && (
        <div className="bg-[#F9F7FF] rounded-lg p-4 border border-[#6B5BEE]/20">
          ...
          <div className="border-l-2 border-[#6B5BEE] pl-3 mb-3">
          ...
          <textarea
            ...
            placeholder="Add a note to describe why this is important..."
          ...
          <Flex alignItems="center" justifyContent="space-between">
            ...
            <Flex gap="SM">
              <ActionButton emphasis="tertiary" size="SM" onClick={() => {
              ...
              <ActionButton emphasis="primary" size="SM" onClick={() => {
...
/**
 * Response table row with expandable functionality (V2 approach)
 * Click to expand into a full card with video and transcript
 */
function ResponseRow({ clipDuration, participantId, responseValue, respondedAt, hasHighlight = false, hideResponse = false, isOpenQuestion = false, isExpanded = false, onToggle }) {
  if (isExpanded) {
    return null; // Expanded card is rendered separately
  }
  
  return (
    <div 
      className={`flex items-center py-4 border-b border-[rgba(108,113,140,0.12)] ${isOpenQuestion ? 'hover:bg-[#F9F7FF] cursor-pointer' : 'hover:bg-neutral-50'}`}
      onClick={isOpenQuestion ? onToggle : undefined}
    >
      ...
      {!hideResponse && (
        <div className="flex-1 px-4">
          <Flex alignItems="center" gap="SM">
            ...
            {isOpenQuestion && (
              <ChevronDown size={16} className="text-[#6C718C]" />
            )}
          </Flex>
        </div>
      )}
        ...
        <div>
          ...
          {activeTab === 'all' && (
            <>
              ...
              {/* Table Rows with Expandable Cards (V2 approach) */}
                  ...
                  <div key={response.id}>
                    {isExpanded && isOpenQuestion ? (
                      ...
                      <ResponseRow 
                        ...
                        onToggle={() => setExpandedResponseId(isExpanded ? null : response.id)}
                      />
                    )}
                  </div>
                );
