 ...
 * @param {string} props.goalDescription - Goal description text (activity only)
 * @param {Function} props.onGoalDescriptionChange - Handler for goal description updates
 * @param {boolean} props.isAIModerated - Whether the goal is AI moderated
 * @param {Function} props.onAIModeratedChange - Handler for AI moderated toggle
 * @param {string} props.scenario - Goal introduction/scenario text (activity only)
 * @param {Function} props.onScenarioChange - Handler for scenario updates
 * @param {boolean} props.isGoalActivity - Whether block is the goal activity
 */
    ...
    return (
      ...
      >
        <Text color="default.main.secondary">
          Select an activity or question to edit its settings
        </Text>
      </Flex>
    );
  }
  ...
  return (
    <Flex flexDirection="column" className="h-full bg-white">
      ...
      <SettingsPanelHeader 
        ...
        iconName={blockType?.iconName || 'website'}
        iconColor={blockType?.arianeColor || 'awake'}
      ...
      <ScrollContainer className="flex-1">
        <Box className="px-4 py-6">
          {isGoalActivity && (
            <Flex flexDirection="column" gap="LG" className="mb-6">
              <Flex justifyContent="space-between" alignItems="center">
                <Flex flexDirection="column" gap="XS">
                  <Text>AI moderated goal</Text>
                  <Text type="caption" color="default.main.secondary">
                    Toggle AI moderation for this goal.
                  </Text>
                </Flex>
                <Toggle
                  size="xs"
                  checked={isAIModerated}
                  onChange={(value) => {
                    console.log('AI moderated toggled:', value);
                    onAIModeratedChange?.(value);
                  }}
                />
              </Flex>
              {isAIModerated && (
                <>
                  <Field label="Goal description" helperText="Defines the goal outcome for this activity.">
                    <TextAreaControl
                      rows={3}
                      ...
                      onChange={(event) => {
                        ...
                        console.log('Goal description updated:', nextDescription);
                        onGoalDescriptionChange?.(nextDescription);
                      }}
                    />
                  </Field>
                  <Field label="Introduction and scenario" helperText="Read aloud before the task begins.">
                    <TextAreaControl
                      rows={4}
                      value={scenario}
                      placeholder="Introduce the scenario participants should imagine..."
                      onChange={(event) => {
                        const nextScenario = event.target.value;
                        console.log('Introduction/scenario updated:', nextScenario);
